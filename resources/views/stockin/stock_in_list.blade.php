		...
		<table class="table table-bordered" id="example1">
			...
			<tbody>
				...
				<tr>
					...
					<td>{{$d->invoiceID}}</td>
					<td>{{$d->discount}}</td>
					<?php $total=App\Models\StockIn::where('invoiceID',$d->invoiceID)->sum('total'); ?>
					<td>{{$total}}</td>
					
					<td>
						<a href="{{url('/stock_in/print/'.$d->invoiceID)}}" class="btn btn-xs btn-primary">View</a> |
						<a href="{{url('/stock_in/edit/'.$d->invoiceID)}}" class="btn btn-xs btn-success">Edit</a> | 
						<a class="btn btn-xs btn-danger" href="{{ url('/stock_in/delete/'.$d->invoiceID) }}"
